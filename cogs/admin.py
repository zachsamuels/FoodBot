...
class Admin:
    ...
    async def mellow(self, ctx, *, code):
        with open("~/mellow/test.mlw", 'w') as f:
            f.write(code)
            f.close()
        def runshell(code):
            with subprocess.Popen(["/bin/bash", "-c", "python3 ~/mellow/mellow.py ~/mellow/test.mlw"], stdout=subprocess.PIPE, stderr=subprocess.PIPE) as process:
